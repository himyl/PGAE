...
class clustering_metrics():
    def __init__(self, true_label, predict_label, log_name):
    ...
    def evaluationClusterModelFromLabel(self):
        ...
        max_f1 = max(f1_macro, f1_micro)
        max_precision = max(precision_macro, precision_micro)

        print('ACC=%f, NMI=%f, F1=%f, Precision=%f, ARI=%f' % (acc, nmi, max_f1, max_precision, adjscore))

        fh = open('log_{}.txt'.format(self.log_name), 'a')

        fh.write('ACC=%f, NMI=%f, F1=%f, Precision=%f, ARI=%f' % (acc, nmi, max_f1, max_precision, adjscore))
        fh.write('\r\n')
        fh.flush()
        fh.close()
