...
from input_data import load_data, load_wiki_data
import inspect
...
def format_data(data_name):
    # Load data
    if data_name.lower() == "wiki":
        adj, features, true_labels = load_wiki_data(file_path="data/wiki-txt/")
    else:
    ...
    items_name = ["adj", "num_features", "num_nodes", "features_nonzero", "pos_weight", "norm", "adj_norm", "adj_label", "features", "true_labels", "train_edges", "val_edges", "val_edges_false", "test_edges", "test_edges_false", "adj_orig"]
    ...
    # add by syh
    for item_name, item in zip(items_name, items):
         feas[item_name] = item  

    return feas

def get_optimizer(model_str, model, discriminator, placeholders, pos_weight, norm, d_real,num_nodes, p_alpha):
    if model_str == 'arga_ae':
        d_fake = discriminator.construct(model.embeddings, reuse=True)
        # parga
        dc_loss_fake = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(d_fake), logits=d_fake,name='dcfake1'))
        fake_grad = tf.gradients(dc_loss_fake, model.embeddings)
        model.embeddings = model.embeddings - p_alpha * fake_grad[0]
        d_fake_p = discriminator.construct(model.embeddings, reuse=True)
        #
...
def retrieve_name(var):
    ...
    result = [var_name for var_name, var_val in callers_local_vars if var_val is var and var_name != "item"] 
    if "adj" in result and "adj_train" in result:
        return "adj"
    else:
